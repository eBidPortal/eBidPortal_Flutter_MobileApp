...
class EnhancedDynamicFieldsForm extends ConsumerStatefulWidget {
  ...
  @override
  EnhancedDynamicFieldsFormState createState() =>
      EnhancedDynamicFieldsFormState();

  /// Access to form validation from parent widgets
  static bool validateForm(BuildContext context) {
    final state = context
        .findAncestorStateOfType<EnhancedDynamicFieldsFormState>();
    return state?.validateForm() ?? false;
  }
}

class EnhancedDynamicFieldsFormState
    extends ConsumerState<EnhancedDynamicFieldsForm> {
  ...
  void _updateControllers() {
    ...
    final fieldsToRemove = _controllers.keys
        .where((fieldName) => !widget.values.containsKey(fieldName))
        .toList();
    ...
    for (final section in template.sections) {
      for (final field in section.fields) {
        ...
        if (field.type == 'object') {
          final objectValue =
              widget.values[field.name] as Map<String, dynamic>? ?? {};
  ...
  Widget build(BuildContext context) {
    final templateAsync = ref.watch(
      categoryTemplateProvider(widget.categoryId),
    );
    ...
    return templateAsync.when(
      data: (template) {
        ...
        // Initialize controllers for template fields (always do this)
      ...
      error: (error, stack) =>
          Center(child: Text('Error loading template: $error')),
    );
  }

  Widget _buildTemplateForm(CategoryTemplate template) {
    // Sort sections by order
    final sortedSections = template.sections
  ...
  Widget _buildSection(TemplateSection section) {
    final isExpanded =
        _expandedSections[section.title] ?? !section.isCollapsible;
    ...
    switch (field.type) {
      case 'text':
        ...
        return TextFormField(
          ...
          onChanged: (value) {
            print(
              'EnhancedDynamicFieldsForm: Text field "${field.name}" changed to: $value',
            );
      ...
      case 'textarea':
        ...
        return TextFormField(
          ...
          onChanged: (value) {
            print(
              'EnhancedDynamicFieldsForm: Textarea field "${field.name}" changed to: $value',
            );
            _onFieldValueChanged(field.name, value);
          },
        );
      ...
      case 'number':
        ...
        return TextFormField(
          ...
          onChanged: (value) {
            final numValue = num.tryParse(value);
            print(
              'EnhancedDynamicFieldsForm: Number field "${field.name}" changed to: $numValue',
            );
      ...
      case 'email':
        ...
        return TextFormField(
          ...
          onChanged: (value) {
            print(
              'EnhancedDynamicFieldsForm: Email field "${field.name}" changed to: $value',
            );
            _onFieldValueChanged(field.name, value);
          },
        );
      ...
      case 'url':
        ...
        return TextFormField(
          ...
          onChanged: (value) {
            print(
              'EnhancedDynamicFieldsForm: URL field "${field.name}" changed to: $value',
            );
            _onFieldValueChanged(field.name, value);
          },
        );
      ...
      case 'phone':
        ...
        return TextFormField(
          ...
          onChanged: (value) {
            print(
              'EnhancedDynamicFieldsForm: Phone field "${field.name}" changed to: $value',
            );
            _onFieldValueChanged(field.name, value);
          },
        );
  ...
  Widget _buildSelectField(TemplateField field, dynamic currentValue) {
    ...
    final hasStaticOptions =
        field.uiConfig.containsKey('options') &&
    ...
    // Ensure the current value is valid, otherwise set to null
    final validValue =
        normalizedOptions.any((option) => option['value'] == currentValue)
        ? currentValue
        : null;
    ...
    return DropdownButtonFormField<String>(
      ...
      onChanged: normalizedOptions.isNotEmpty
          ? (value) {
              _onFieldValueChanged(field.name, value);
            }
          : null,
  ...
  void _onFieldValueChanged(String fieldName, dynamic value) {
    ...
    if (fieldName.contains('.')) {
      ...
      final currentParentValue =
          widget.values[parentFieldName] as Map<String, dynamic>? ?? {};
  ...
  void _reloadDependentFields(String changedFieldName, dynamic newValue) {
    ...
    for (final dependentField in allDependentFields) {
      print(
        'EnhancedDynamicFieldsForm: Reloading options for dependent field "$dependentField" due to change in "$changedFieldName"',
      );
      ...
      templateAsync.when(
          ...
          if (template != null) {
            ...
            if (dependentTemplateField != null) {
              final dynamicOptions =
                  dependentTemplateField.uiConfig['dynamic_options']
                      as Map<String, dynamic>?;
              final autoDynamicOptions = _getAutoDynamicOptions(
                dependentField,
                dependentTemplateField,
              );
              ...
              if (options != null) {
                ...
                print(
                  'EnhancedDynamicFieldsForm: Reloading "$dependentField" with updated values: $updatedValues',
                );
        ...
        error: (error, stack) {
          print(
            'EnhancedDynamicFieldsForm: Error reloading dependent fields: $error',
          );
        },
      );
    }
  ...
  Widget _buildRangeField(TemplateField field, dynamic currentValue) {
    final rangeValue =
        currentValue as num? ?? field.uiConfig['defaultValue'] ?? 50;
  ...
  Widget _buildColorField(TemplateField field, dynamic currentValue) {
    final colorValue =
        currentValue as String? ?? field.uiConfig['defaultValue'] ?? '#FF0000';
    ...
    return Row(
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            color: Color(
              int.parse(colorValue.replaceFirst('#', ''), radix: 16) +
                  0xFF000000,
            ),
  ...
  TemplateField _createSubFieldFromJson(
        ...
        children: [
          ...
          Material(
            child: TextFormField(
              ...
              decoration: InputDecoration(
                hintText:
                    'Enter values separated by commas (e.g., Option 1, Option 2, Option 3)',
                border: const OutlineInputBorder(),
                contentPadding: const EdgeInsets.symmetric(
                  horizontal: 12,
                  vertical: 16,
              ...
              onChanged: (value) {
                ...
                final parsedValues = value
                    ...
                    .toList();
                print(
                  'EnhancedDynamicFieldsForm: Checkbox field "${field.name}" changed to: $parsedValues',
                );
          ...
          ...options.map((option) {
            ...
            return CheckboxListTile(
              ...
              onChanged: (bool? checked) {
                ...
                print(
                  'EnhancedDynamicFieldsForm: Checkbox field "${field.name}" changed to: $newSelectedValues',
                );
      ...
      children: [
        ...
        ElevatedButton.icon(
          onPressed: () {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Array field management not implemented yet'),
              ),
            );
          },
  ...
  void _loadDynamicOptionsForTemplate(CategoryTemplate template) {
    ...
    for (final section in template.sections) {
      for (final field in section.fields) {
        final dynamicOptions =
            field.uiConfig['dynamic_options'] as Map<String, dynamic>?;
        final hasStaticOptions =
            field.uiConfig.containsKey('options') &&
            (field.uiConfig['options'] as List?)?.isNotEmpty == true;

        // Don't use auto-configuration if field has static options
        final autoDynamicOptions = hasStaticOptions
            ? null
            : _getAutoDynamicOptions(field.name, field);
        ...
        if (options != null) {
          ...
          if (dependsOn != null) {
            ...
            print(
              'EnhancedDynamicFieldsForm: Field "${field.name}" depends on "$dependsOn"',
            );
            // Load options for dependent fields if parent has a value
            if (widget.values[dependsOn] != null &&
                widget.values[dependsOn].toString().isNotEmpty) {
              final existingOptions = _dynamicOptions[field.name];
              if (existingOptions == null || existingOptions.isEmpty) {
                print(
                  'EnhancedDynamicFieldsForm: Initial load for dependent field "${field.name}" with parent "$dependsOn" value: ${widget.values[dependsOn]}',
                );
  ...
  Map<String, dynamic>? _getAutoDynamicOptions(
    String fieldName,
    TemplateField field,
  ) {
    ...
    switch (fieldName) {
      ...
      case 'variant':
        return {
          ...
          'data_path':
              'data.variants', // Updated to use nested path for variants
          'label_field': 'name',
          'value_field': 'id',
          'depends_on': 'model',
        };
  ...
  Future<void> _loadOptionsForField(
    String fieldName,
    Map<String, dynamic> dynamicOptions, [
    Map<String, dynamic>? currentValuesParam,
  ]) async {
    print(
      'EnhancedDynamicFieldsForm: _loadOptionsForField called for field: $fieldName',
    );
    ...
    if (dependsOn != null) {
      ...
      print(
        'EnhancedDynamicFieldsForm: Checking dependency for "$fieldName" - depends_on: "$dependsOn", currentValues: $currentValues, dependentValue: "$dependentValue"',
      );
      if (dependentValue == null || dependentValue.isEmpty) {
        print(
          'EnhancedDynamicFieldsForm: Skipping load for dependent field "$fieldName" - "$dependsOn" not selected',
        );
        setState(() {
    ...
    if (existingOptions != null && existingOptions.isNotEmpty) {
      print(
        'EnhancedDynamicFieldsForm: Options already loaded for field "$fieldName", skipping API call',
      );
      return;
    }
    ...
    if (_loadingOptions[fieldName] == true) {
      print(
        'EnhancedDynamicFieldsForm: Already loading options for field "$fieldName", skipping duplicate request',
      );
      return;
    }
    ...
    try {
      ...
      for (final match in matches) {
        ...
        print(
          'EnhancedDynamicFieldsForm: Replacing {$varName} with "$varValue" in URL',
        );
        apiUrl = apiUrl.replaceAll('{$varName}', varValue);
      }
      print(
        'EnhancedDynamicFieldsForm: URL after template replacement: $apiUrl',
      );
      ...
      if (apiUrl.startsWith('https://api.ebidportal.com/api/v1/')) {
        ...
        print(
          'EnhancedDynamicFieldsForm: Converted full URL to relative path: $apiUrl',
        );
      }

      print(
        'EnhancedDynamicFieldsForm: Loading options for field "$fieldName" from URL: $apiUrl',
      );
      ...
      print(
        'EnhancedDynamicFieldsForm: Field: $fieldName, Data path: $dataPath, Label field: $labelField, Value field: $valueField',
      );
      print(
        'EnhancedDynamicFieldsForm: Current values for API call: $currentValues',
      );

      final response = await apiClient.get(apiUrl);

      print(
        'EnhancedDynamicFieldsForm: Received response for field "$fieldName": status=${response.statusCode}',
      );
      print(
        'EnhancedDynamicFieldsForm: Response headers: ${response.headers.map}',
      );
      ...
      if (response.data['success'] == true) {
        ...
        print(
          'EnhancedDynamicFieldsForm: Found ${data.length} options for field "$fieldName"',
        );
        ...
        print(
          'EnhancedDynamicFieldsForm: Successfully loaded ${options.length} options for field "$fieldName"',
        );
      ...
      print(
        'EnhancedDynamicFieldsForm: Error loading options for field "$fieldName"',
      );
      ...
      if (e is DioException) {
        print(
          'EnhancedDynamicFieldsForm: DioException details for field "$fieldName":',
        );
      ...
      try {
        ...
        templateAsync.when(
          data: (template) {
            if (template != null) {
              ...
              if (templateField != null &&
                  (templateField.uiConfig.containsKey('options') ||
                      templateField.validation.containsKey('options'))) {
                final staticOptions =
                    templateField.uiConfig['options'] as List<dynamic>? ??
                ...
                if (staticOptions != null && staticOptions.isNotEmpty) {
                  ...
                  setState(() {
                    _dynamicOptions[fieldName] = options;
                    _loadingOptions[fieldName] = false;
                    _optionsErrors[fieldName] =
                        null; // Clear error since we have fallback
                  });

                  print(
                    'EnhancedDynamicFieldsForm: Using static fallback options for field "$fieldName": ${options.length} options',
                  );
                  fallbackUsed = true;
                }
              }
          ...
          error: (error, stack) {
            print(
              'EnhancedDynamicFieldsForm: Error accessing template for fallback: $error',
            );
          },
        );
      } catch (fallbackError) {
        print(
          'EnhancedDynamicFieldsForm: Error setting up fallback for field "$fieldName": $fallbackError',
        );
      }
      ...
      if (!fallbackUsed) {
        ...
        if (e is DioException) {
          errorMessage +=
              ': ${e.response?.statusCode ?? 'Network Error'} - ${e.message ?? 'Unknown error'}';
        } else {
          errorMessage += ': $e';
        }
  ...
  List<dynamic> _extractDataFromResponse(
